...
WORK=/tmp/rg3-live-v033
PUBLIC=/tmp/rg3-public-v033
PRIVATE=/tmp/rg3-private-v033
STRIPE_HOME=/tmp/rg3-stripe-home-v033
STRIPE_XDG=/tmp/rg3-stripe-xdg-v033
SECRET=""
CURRENT_PHASE="BOOT"
...
# Match Stripe CLI's own canary isolation contract exactly: XDG_CONFIG_HOME plus
# STRIPE_CONFIG_DIR, with a fresh empty config.toml. This prevents ambient profile
# state from selecting the active-sandbox or browser-login branches.
export HOME="$STRIPE_HOME" XDG_CONFIG_HOME="$STRIPE_XDG" STRIPE_CONFIG_DIR="$STRIPE_XDG/stripe"
export STRIPE_CLI_TELEMETRY_OPTOUT=1
rm -rf "$HOME" "$XDG_CONFIG_HOME"
mkdir -p "$HOME" "$STRIPE_CONFIG_DIR"
: > "$STRIPE_CONFIG_DIR/config.toml"
EMAIL="$(git -C "$REPO_ROOT" log -1 --format=%ae)"
case "$EMAIL" in *@*) ;; *) record_failure "$CURRENT_PHASE"; write_state "$CURRENT_PHASE" FAIL; seal_public; exit 2;; esac
...
if ! "$STRIPE_BIN" sandbox create --email "$EMAIL" --non-interactive >"$RAW" 2>"$ERR"; then
  record_failure "$CURRENT_PHASE" "$ERR"; write_state "$CURRENT_PHASE" FAIL; seal_public; exit 2
fi
PARSE_ERR="$PRIVATE/sandbox-parse.err"
if ! python - "$RAW" "$STRIPE_CONFIG_DIR/config.toml" "$PRIVATE" "$PUBLIC/SANDBOX_PROVISION_RECEIPT.json" 2>"$PARSE_ERR" <<'PY'
from pathlib import Path
import json,os,sys,tomllib
raw=Path(sys.argv[1]); config_path=Path(sys.argv[2]); private=Path(sys.argv[3]); receipt=Path(sys.argv[4])
text=raw.read_text(errors='replace')

def decoded_objects(text):
...
def walk_key(obj,key):
    ...
    if isinstance(obj,dict):
        for k,v in obj.items():
            if k==key and isinstance(v,str) and v: vals.append(v)
            vals.extend(walk_key(v,key))
    elif isinstance(obj,list):
        for v in obj: vals.extend(walk_key(v,key))
    return vals

objs=decoded_objects(text)
stdout_secrets=[]
for obj in objs: stdout_secrets.extend(walk_key(obj,'secret_key'))
stdout_secrets=list(dict.fromkeys(stdout_secrets))
config={}
if config_path.exists() and config_path.stat().st_size:
    with config_path.open('rb') as f: config=tomllib.load(f)
config_test=list(dict.fromkeys(walk_key(config,'test_mode_api_key')))
config_live=list(dict.fromkeys(walk_key(config,'live_mode_api_key')))
claim_urls=list(dict.fromkeys(walk_key(config,'sandbox_claim_url')))
expires=list(dict.fromkeys(walk_key(config,'sandbox_expires_at')))
pubs=list(dict.fromkeys(walk_key(config,'test_mode_pub_key')))

# A disposable CLI-created sandbox must be claimable (rkcs_), must have no live key,
# and must leave claim metadata in the isolated config written by saveSandboxToConfig.
candidates=list(dict.fromkeys([s for s in stdout_secrets+config_test if s.startswith('rkcs_')]))
if len(candidates)!=1:
    raise SystemExit(f'expected exactly one claimable-sandbox credential candidate; found {len(candidates)}')
if any(config_live):
    raise SystemExit('refused: isolated sandbox config contains a live-mode API key')
if not claim_urls:
    raise SystemExit('refused: successful command did not leave sandbox_claim_url provenance')
if not expires:
    raise SystemExit('refused: successful command did not leave sandbox_expires_at provenance')
secret=candidates[0]
pub=pubs[0] if len(pubs)==1 else ''
claim=claim_urls[0] if len(claim_urls)==1 else ''
for name,val in [('secret',secret),('publishable',pub),('claim_url',claim)]:
    p=private/name; p.write_text(val); os.chmod(p,0o600)
(private/'prefix').write_text('rkcs_')
source='stdout+config' if secret in stdout_secrets and secret in config_test else ('stdout' if secret in stdout_secrets else 'isolated_config')
receipt.write_text(json.dumps({
  ...
  'credential_class':'rkcs_','credential_source':source,
  'isolated_config_path_class':'STRIPE_CONFIG_DIR/stripe-config',
  'claimable_claim_url_present':bool(claim_urls),
  'claimable_expiry_present':bool(expires),
  'live_mode_key_present_in_isolated_config':False,
  'primary_evidence_consumed':False
},sort_keys=True,separators=(',',':')))
PY
then
  record_failure "$CURRENT_PHASE" "$PARSE_ERR"; write_state "$CURRENT_PHASE" FAIL; seal_public; exit 2
fi
...
WHOAMI_ERR="$PRIVATE/whoami-verify.err"
if ! python - "$PRIVATE/whoami.json" "$PUBLIC/SANDBOX_PROVISION_RECEIPT.json" 2>"$WHOAMI_ERR" <<'PY'
from pathlib import Path
import json,sys
...
PY
then
  record_failure "$CURRENT_PHASE" "$WHOAMI_ERR"; write_state "$CURRENT_PHASE" FAIL; seal_public; exit 2
fi
export RG3_STRIPE_CLAIMABLE_SANDBOX_VERIFIED=1
write_state "$CURRENT_PHASE" PASS
...
seal_public
write_state "$CURRENT_PHASE" PASS
# Re-seal after the final state update so the index commits the final state.
seal_public
rm -rf "$PRIVATE/oracle"
echo 'PRIVATE_ORACLE_DESTROYED_BEFORE_UPLOAD=TRUE'
echo 'RG3_STRIPE_E0_V033=PASS; PRIMARY_EVIDENCE=0'
