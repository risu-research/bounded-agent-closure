...
WORK=/tmp/rg3-shadow-v034
OUT=/tmp/rg3-shadow-v034-public
...
python "$BASE/patch_e0_v034.py" --script "$CAP/scripts/stripe_e0.py" --receipt "$OUT/E0_V034_PATCH_RECEIPT.json"
(cd "$CAP" && pytest -q tests/test_capsule.py)
python "$BASE/shadow_stripe_e0_v034.py" \
  --executor "$CAP/scripts/stripe_e0.py" --sealed-root "$SEALED" --out "$OUT/shadow"
python - "$OUT/shadow/SHADOW_E0_V034_RESULT.json" <<'PY'
from pathlib import Path
import json,sys
r=json.loads(Path(sys.argv[1]).read_text())
assert r['status']=='PASS'
assert r['real_provider_calls']==0
assert r['real_payment_mutations']==0
assert r['primary_evidence_consumed'] is False
assert r['stable_rescan_race']['retried'] is True
assert r['completeness_negative_control']=={'complete_J2':'FAIL','truncated_J2':'UNKNOWN'}
print('RG3_SHADOW_E0_V034=PASS; REAL_PROVIDER_CALLS=0; PRIMARY_EVIDENCE=0')
PY
