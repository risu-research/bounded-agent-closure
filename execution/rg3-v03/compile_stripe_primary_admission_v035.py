...
SCENARIOS=[
 'S1_ORDINARY_REFUND_SUCCESS',
 'S2_SAME_KEY_RETRY_AFTER_RESPONSE_GUILLOTINE',
 'S3_DIFFERENT_KEY_DUPLICATE_NEGATIVE_CONTROL',
 'S4_ASYNC_PENDING_TO_SUCCEEDED',
 'S5_ASYNC_SUCCEEDED_TO_FAILED',
 'S6_PARTIAL_EVIDENCE_COMPLETENESS_REMOVAL',
...
def main()->int:
    ...
    ap.add_argument('--e05-lock',type=Path,required=True)
    ap.add_argument('--e05-manifest',type=Path,required=True)
    ap.add_argument('--final-seal',type=Path,required=True)
    ap.add_argument('--out-dir',type=Path,required=True)
    a=ap.parse_args()
    for p in (a.e05_lock,a.e05_manifest,a.final_seal):
        if not p.is_file(): raise SystemExit(f'admission refused: missing {p}')
        no_secret(p)
    lock=load(a.e05_lock); man=load(a.e05_manifest); seal=load(a.final_seal)
    ...
    # The preprimary final seal is used as an immutable provenance anchor; do not infer a primary outcome from it.
    if seal.get('source_commit') not in (None,FROZEN_SOURCE_COMMIT):
        raise SystemExit('admission refused: frozen source commit mismatch')
    # Compile only an admission precommit. It deliberately contains no PaymentIntent/refund IDs and performs no provider call.
    precommit={
      'status':'STRIPE_PRIMARY_ADMISSION_PRECOMMIT_ONLY',
      'primary_execution_authorized':False,
      'reason':'Scenario parameters/oracle-vault implementation must be separately frozen and independently qualified before authorization.',
      'provider':'stripe','api_version':EXPECTED_API,
      'frozen_source_commit':FROZEN_SOURCE_COMMIT,'frozen_stripe_binding':FROZEN_BINDING,
      ...
      'calibration':{
        'D_obs_seconds':lock['D_obs_seconds'],'H_check_seconds':lock['H_check_seconds'],'observation_max_gap_seconds':lock['observation_max_gap_seconds']},
      'scenario_order':SCENARIOS,
      'nonnegotiable_rules':[
        'fresh_original_payment_per_scenario_unless_scenario_requires_repeated_attempts',
        'same_admissible_raw_evidence_across_projectx_specialist_conventional_strong_arms',
        'guillotine_upstream_response_oracle_only_until_all_arm_verdict_hashes_commit',
        'projectx_specialist_conventional_strong_verdicts_hash_frozen_before_oracle_reveal',
        'no_same_target_outcome_aware_semantic_or_parser_repair_after_primary_evidence',
        'J8_ECONOMIC_FINALITY_expected_UNKNOWN_in_sandbox',
        'provider_event_delivery_order_never_causal_or_completeness_proof',
        'no_primary_rerun_after_outcome_relevant_evidence_except_narrow_preobservation_harness_abort'
      ],
    ...
    print(json.dumps({'status':'PASS','authorization':False,'precommit_sha256':manifest['STRIPE_PRIMARY_ADMISSION_PRECOMMIT.json'],'provider_calls':0,'primary_evidence_consumed':False},sort_keys=True))
    return 0
